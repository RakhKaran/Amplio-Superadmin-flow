...
import { useGetBusinessAddressDetails } from 'src/api/companyKyc';
...
export default function CompanyAddressVerification({ companyProfile }) {
  const companyId = companyProfile?.data?.id;
  const { registeredAddress, correspondenceAddress, loading, refreshAddressDetails } = useGetBusinessAddressDetails(companyId);
  ...
  const submitVerification = async (nextStatus, reason = '') => {
    try {
      ...
      const response = await axiosInstance.patch('/company-profiles/business-address-verification', {
        companyId,
        status: nextStatus,
        reason,
      });
...
CompanyAddressVerification.propTypes = {
