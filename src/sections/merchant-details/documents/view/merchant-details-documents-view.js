...
import { useGetDocuments } from 'src/api/merchant-kyc';
import PropTypes from 'prop-types';

export default function MerchantDetailsDocuments({ merchant }) {
  const merchantId = merchant?.id;

  const { documents = [], refreshDocuments } = useGetDocuments(merchantId);
  ...
  const handleApprove = async (documentId) => {
    try {
      await axiosInstance.patch('/merchant-profiles/document-verification', {
        status: 1,
        documentId,
        reason: '',
      });
            ...
            <TableBody>
              {documents.length === 0 ? (
                ...
                documents.map((doc) => (
                  <TableRow key={doc.id}>
                    <TableCell>
                      {doc.merchantKycDocumentRequirements?.documentLabel || 'NA'}
                    </TableCell>
                    ...
                    <TableCell align="right">
                      <IconButton
                        ...
                        onClick={() => {
                          ...
                          link.href = doc.media?.fileUrl;
                          link.download = doc.media?.fileName || 'document';
                          link.target = '_blank';
                          link.click();
                        }}
                      >
                      ...
                      {/* <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                        <IconButton
                          ...
                          sx={{
                            ...
                            cursor:
                              doc.status === 1 || doc.status === 2 ? 'not-allowed' : 'pointer',
                          }}
                        >
...
MerchantDetailsDocuments.propTypes = {
