...
import FraudAlertTableRow from '../fraud-alert-table-row';
...
export default function FraudAlertListView({ fraudAlertsData }) {
  ...
  const [tableData, setTableData] = useState(fraudAlertsData || []);

  const [filters, setFilters] = useState(defaultFilters);

  useEffect(() => {
    setTableData(fraudAlertsData || []);
  }, [fraudAlertsData]);
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (item) =>
        item.alertId?.toLowerCase().includes(name.toLowerCase()) ||
        item.merchant?.toLowerCase().includes(name.toLowerCase()) ||
        item.reason?.toLowerCase().includes(name.toLowerCase()) ||
