...
import MerchantProfilesTableRow from '../merchant-profiles-table-row';
import MerchantProfileTableToolbar from '../merchant-profiles-table-toolbar';
import MerchantProfileTableFiltersResult from '../merchant-profiles-table-filters-result';
import { useFilterMerchantProfiles, useGetMerchantProfiles } from 'src/api/merchant-profiles';
...
export default function MerchantProfileListView() {
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.merchant.details(id));
    },
    [router]
  );
  ...
  const handleDeleteRows = useCallback(() => {
    table.onUpdatePageDeleteRows({
      ...
      totalRowsInPage: filteredMerchantProfiles.length,
      totalRowsFiltered: filteredMerchantProfiles.length,
    });
  }, [filteredMerchantProfiles.length, table, tableData]);
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Merchant Profiles', href: paths.dashboard.merchant.list },
            { name: 'List' },
          ]}
          sx={{
            mb: { xs: 3, md: 5 },
        ...
        <Card>
          ...
          <MerchantProfileTableToolbar filters={filters} onFilters={handleFilters} />

          {canReset && (
            <MerchantProfileTableFiltersResult
              filters={filters}
              onFilters={handleFilters}
              onResetFilters={handleResetFilters}
              results={filteredMerchantProfiles.length}
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            ...
            <Scrollbar>
              <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 960 }}>
                ...
                <TableBody>
                  {filteredMerchantProfiles.map((row) => (
                    <MerchantProfilesTableRow
                      key={row.id}
                      row={row}
